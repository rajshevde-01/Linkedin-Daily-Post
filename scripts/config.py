...
# --- Post Settings ---
POST_MIN_WORDS = 300
POST_MAX_WORDS = 500  # Targeted for 300-500 word range
...
STRICT RULES:
- Length: {POST_MIN_WORDS}-{POST_MAX_WORDS} words. THIS IS NON-NEGOTIABLE. Count your words carefully. You MUST write a FULL, DETAILED post of at least {POST_MIN_WORDS} words but no more than {POST_MAX_WORDS} words. Posts under {POST_ABSOLUTE_MIN} words are AUTOMATICALLY DELETED. Posts over {POST_MAX_WORDS} words are AUTOMATICALLY TRIMMED. Do NOT write a skeletal outline — write complete sentences, full paragraphs, and technical depth. A 50-word post = IMMEDIATE REJECTION.
...
- **LENGTH**: You MUST hit the {POST_MIN_WORDS}-{POST_MAX_WORDS} word range. THIS IS THE SINGLE MOST IMPORTANT RULE. If your draft is under {POST_MIN_WORDS} words, you MUST expand it before outputting. If your draft exceeds {POST_MAX_WORDS} words, you MUST trim it. Add technical analysis of the 'How' and 'Why', include a '🛡️ Operational Checklist' section, deepen the contrast framework, or add a practitioner micro-story. A post under {POST_ABSOLUTE_MIN} words is an AUTOMATIC FAILURE.
...
1. **WORD COUNT IS #1 PRIORITY**: You MUST write {POST_MIN_WORDS}-{POST_MAX_WORDS} words of SUBSTANTIVE content. Count carefully before submitting. Under {POST_ABSOLUTE_MIN} = DELETED. Under {POST_MIN_WORDS} = REJECTION. Over {POST_MAX_WORDS} = TRIM REQUIRED. A post with only 50-80 words is a CATASTROPHIC FAILURE. Write FULL paragraphs with technical depth.
